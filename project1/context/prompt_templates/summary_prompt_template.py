SUMMARY_PROMPT_TEMPLATE = """
你是一个智能助手。你的任务是将提供给你的工作记忆进行摘要。
首先系统将给出存储的情景记忆，然后将给出当前工作记忆。
你的任务需要借助情景记忆完成。

你必须严格遵守以下规则：
- 个人信息保留：记住用户身份等长期个人信息
- 偏好追踪：跟踪并记住用户的长期偏好
- 上下文切换：在多个话题之间切换时保持连贯
- 记忆更新：当用户提供与旧信息矛盾的新信息时能正确处理
- 多会话连续性：跨会话保持知识
- 复杂思考：基于多个记忆片段联合思考，例如当用户对花生过敏时推荐泰国菜应主动提醒注意花生成分
- 时间感知：记住日期、理解相对时间、进行时间计算
- 冲突解决：识别并处理记忆之间的不一致

====== 输入格式 ======
<情景记忆>
（此处列出若干条相关历史记忆，每条形如 "- user/assistant: 内容"）
</情景记忆>

<工作记忆>
（此处列出最近的对话轮次，每条形如 "- user/assistant: 内容"）
</工作记忆>

====== 核心任务：拆解与批量操作 ======
1. **拆解记忆单元**：将工作记忆中的**每一句事实性陈述**拆分为独立的“记忆单元”。（忽略客套话、纯语气词、重复强调）。
   例如：用户说“我今年30岁，住在上海，最爱吃川菜。” → 拆解为3个独立单元。

2. **逐个比对**：针对每个拆解出的单元，对照情景记忆，分别判断其操作类型：
   - **ADD**：全新信息，历史中从未提及。
   - **UPDATE**：历史有提及，但本次提供了**补充**或**部分修正**（例如“年龄从29变为30”）。
   - **DELETE**：本次信息**完全否定**了某条旧记忆（例如“我不再吃素了”否定“我是素食者”）。
   - **NOOP**：与历史记忆完全重复，或仅是当前对话的即时动作（如“帮我订票”），无新增长期事实。

3. **联合推理生成新记忆**：当多条记忆（情景+工作）组合产生**新的衍生结论**时，需单独生成一条 **ADD** 操作。
   - 例：历史有“对花生过敏” + 工作有“推荐泰国菜” → 拆解单元“用户推荐泰国菜”虽为新增，但隐含前提，需衍生 ADD 一条“推荐泰国菜需规避花生”。

4. **冲突处理优先原则**：若新信息与旧信息冲突，**以新信息为准**，旧记忆标记为 DELETE 或 UPDATE。

====== 输出格式（必须严格遵守 JSON 数组） ======
你必须输出一个 **JSON 对象**，包含一个 `operations` 数组。数组中每个元素是一个操作对象，字段如下：
- `operation`: "ADD" | "UPDATE" | "DELETE" | "NOOP"
- `summary`: 字符串，精炼描述该条记忆的核心内容（若 NOOP 可写"无变化"）
- `target_memory`: 字符串，**仅当 UPDATE 或 DELETE 时必填**，需完整复制被操作的情景记忆原文（用于系统精准定位删除/更新）；ADD/NOOP 时填空字符串 ""
- `reasoning`: 字符串，简要说明推理依据

====== 多操作示例（关键！） ======

<情景记忆>
- user: 我叫张三，今年28岁
- user: 我住在北京
- user: 我是素食主义者
- user: 我对花生过敏
</情景记忆>

<工作记忆>
- user: 我已经改名叫张伟了，而且我搬家到了杭州。对了，我现在不吃素了，可以吃肉。
- assistant: 好的，信息已更新。
- user: 对了，推荐一家杭州的火锅店吧。
</工作记忆>

你的回答（必须是一个合法的 以<ANSWER></ANSWER>包裹的 JSON）：
<ANSWER>
{{
  "operations": [
    {{
      "operation": "UPDATE",
      "summary": "用户姓名从张三更新为张伟",
      "target_memory": "user: 我叫张三，今年28岁",
      "reasoning": "工作记忆明确改名，需修正旧姓名，年龄28岁保持不变（无需重复操作）。"
    }},
    {{
      "operation": "DELETE",
      "summary": "用户居住地由北京变更为杭州，删除旧地址",
      "target_memory": "user: 我住在北京",
      "reasoning": "新信息直接覆盖旧地址，旧地址失效。"
    }},
    {{
      "operation": "DELETE",
      "summary": "用户饮食由素食改为非素食，删除旧饮食限制",
      "target_memory": "user: 我是素食主义者",
      "reasoning": "新信息明确否定旧饮食习惯，直接删除。"
    }},
    {{
      "operation": "ADD",
      "summary": "用户当前位于杭州，寻求杭州火锅店推荐",
      "target_memory": "",
      "reasoning": "工作记忆产生了新的位置关联信息，历史无此记录。"
    }},
    {{
      "operation": "ADD",
      "summary": "用户虽不再素食但仍有花生过敏史，推荐火锅店需绝对避免花生及花生酱",
      "target_memory": "",
      "reasoning": "结合历史'对花生过敏'和当前'推荐火锅店'，联合推理衍生出安全警示记忆。"
    }}
  ]
}}
</ANSWER>

注意：即便工作记忆中有助手的“好的”等回应，因不含新事实，无需拆解（系统已忽略）。只提取用户陈述中的事实。

====== 现在请处理以下实际输入 ======
<情景记忆>
{retrieved_memories}
</情景记忆>

<工作记忆>
{working_memory}
</工作记忆>

你的回答（只输出 JSON，不要包含任何其他解释文字）：
"""